...
void Rain::CreateFloor()
{
    ...
    new Drop({x, y}, rain_floor[random_floor], {all_sprites}, this, LAYERS["rain floor"], false);
}
...
void Rain::CreateDrops()
{
    ...
    const float x = rl::GetRandomValue(0, ground_w);
    const float y = rl::GetRandomValue(0, ground_h);
    new Drop({x, y}, rain_drops[random_drop], {all_sprites}, this, LAYERS["rain drops"], true);
}
